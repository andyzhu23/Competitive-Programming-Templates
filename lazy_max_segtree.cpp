...
#include <algorithm>
namespace cp {
#ifndef CP_LAZY_MAX_SEGTREE
#define CP_LAZY_MAX_SEGTREE
template <typename T> class lazy_max_segtree {
    static constexpr int lc(int rt) { return rt << 1; }
    static constexpr int rc(int rt) { return rt << 1 | 1; }

    void push_up(int rt) {
        st[rt] = std::max(st[lc(rt)], st[rc(rt)]);
    }
    ...
    T query(int rt, int l, int r, int x, int y) {
        ...
        else return std::max(query(lc(rt), l, mid, x, mid), query(rc(rt), mid + 1, r, mid + 1, y));
    }

public:
    lazy_max_segtree(int n) : n(n), st((n << 2) + 5), lazy((n << 2) + 5) {}
    lazy_max_segtree(const std::vector<T>& a) : n((int)a.size()), st((n << 2) + 5), lazy((n << 2) + 5) {
        build(1, 1, n, a);
    }
