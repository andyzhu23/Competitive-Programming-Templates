...
class MCMF {
    ...
    bool spfa() {
        ...
        return dist[sink_node] != inf;
    }
    ll dfs(int u, ll f) {
        if(u == sink_node) { total_cost += dist[sink_node] * f; return f; }
        vis[u] = 1;
        ll ret = 0;
...
public:
    // Allocates room for graph nodes 1..n plus source_node=0 and sink_node=n+1. Edges grow dynamically via add().
    MCMF(int n) : source_node(0), sink_node(n + 1), e(2), head(n + 3), cur(n + 3), dist(n + 3), vis(n + 3) {}
    void reserve_edges(int m) { e.reserve(2 * m + 2); } // pre-allocate for m user-visible edges (each occupies 2 internal slots)

    int source() const { return source_node; }
    int sink() const { return sink_node; }
    void set_source(int s) { source_node = s; }
    void set_sink(int t) { sink_node = t; }
    ...
    ll max_flow() {
        if (!computed) {
            while(spfa()) total_flow += dfs(source_node, inf);
        }
        computed = true;
        return total_flow;
    }
    ll cost() {
        if (!computed) max_flow();
        return total_cost;
    }
